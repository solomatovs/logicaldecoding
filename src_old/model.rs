...
use clap::{AppSettings, Parser, FromArgMatches, Args};
...
pub struct PgConnectorOpt
{
    ...
    #[clap(long, env = "PG_DATABASE_URL", hide_env_values = true)]
    pub pg_url: Url,
...
pub struct ChConnectorOpt {
    /// The URL of the postgres leader database to stream from
    ///
    /// Ex: postgres://user:password@localhost:5432/database_name
    #[clap(long, env = "CH_DATABASE_URL", hide_env_values = true)]
    pub ch_url: Url,
}
...
impl TryFrom<Row> for CreateReplicationSlotResult {
    type Error = anyhow::Error;

    fn try_from(value: Row) -> Result<Self, Self::Error> {
      let slot_name = value.try_get(0)?;
      let consistent_point = value.try_get(1)?;
      let snapshot_name = value.try_get(2).unwrap_or(None);
      let output_plugin = value.try_get(3).unwrap_or(None);
...
impl TryFrom<Vec<Row>> for CreateReplicationSlotResult {
  ...
  fn try_from(mut value: Vec<Row>) -> Result<Self, Self::Error> {
    if value.is_empty() {
      Err(CreateReplicationSlotError::EmptyResult.into())
    } else {
      let x = value.remove(0);
      let x = Self::try_from(x);
      x
    }
  }
}
